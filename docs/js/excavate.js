const game = {
	 ...
	 */
	newPile: ( name ) => {
		// We deal thirteen cards from the deck onto piles 1, 2, 3, and 4.
		if ( name.indexOf( '-' ) !== -1 ) {
			// Deal from the pack
			let hand = []
			for ( let i=0; i<13; i++ ) {
	...
	canClickOrDragFromPileAtXY: ( pile, x, y ) => {		
		// The spare can be dragged from if it has cards in it and at least one pile is empty.
		if ( pile.name === 'spare' && pile.cards.length === 1 ) {
	...
	canDropCardAtXYOnPile: ( card, x, y, pile ) => {
		...
		if ( pile.name === 'discard' && cardUI.xyIsInBounds( x, y, pile.elem ) ) {
			...
			let topCard = dealer.peekTopOfPile( 'spare' )
			if ( topCard && topCard.suit === card.suit && (topCard.ordValue > card.ordValue || (topCard.ordValue === 0 && card.ordValue !== 0)) ) {
				return table.outcomes.PILE_IS_INTERACTIVE
			}
		}

		// You can drop anything on the spare if the spare is empty.
		if ( pile.name === 'spare' && pile.cards.length === 0 && cardUI.xyIsInBounds( x, y, pile.elem ) ) {
			return table.outcomes.PILE_IS_INTERACTIVE
		}

		// The spare can be returned to anywhere
		let fromPile = card.elem.getAttribute('data-pile')
		if ( fromPile === 'spare' && pile.name.startsWith( 'tower-' ) ) {
			let topCard = dealer.peekTopOfPile( pile.name )
			if ( topCard && cardUI.xyIsInBounds( x, y, topCard.elem ) ) {
		...
		let legalCard = true //card.ordValue === 0 
...
/*
		// Can't win if there's something in the spare.
		if ( dealer.piles['spare'].cards.length === 1 ) {
		}

		if ( dealer.piles['spare'].cards.length === 0 ) {
			return { state: table.gameOverStates.KEEP_PLAYING }
		}
